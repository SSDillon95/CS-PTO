...
/** Must live outside the parent so React keeps focus across re-renders. */
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) onNotify("error", (error as Error).message);
      } finally {
    ...
    // Load once on mount only — do not re-fetch when parent callbacks change
